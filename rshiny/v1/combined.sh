...
# create-all-files.sh - Automatically create all files for the Shiny S3 Docker project

echo "============================================"
echo "Creating Shiny S3 Docker Project Structure"
echo "============================================"
echo

# Create main directory
PROJECT_DIR="shiny-s3-docker"
...
# Create shiny-server-entrypoint.sh
echo "Creating shiny-server-entrypoint.sh..."
cat > shiny-server-entrypoint.sh << 'EOF'
...
echo "Creating shiny-server.conf..."
cat > shiny-server.conf << 'EOF'
run_as shiny;
server {
  listen 3838;
  location / {
...
# Create cloudguard-root.crt (placeholder)
echo "Creating cloudguard-root.crt placeholder..."
cat > cloudguard-root.crt << 'EOF'
-----BEGIN CERTIFICATE-----
...
# Create rocker scripts
echo "Creating rocker_scripts/install_R_source.sh..."
cat > rocker_scripts/install_R_source.sh << 'EOF'
#!/bin/bash
set -e
...
echo "Creating rocker_scripts/setup_R.sh..."
cat > rocker_scripts/setup_R.sh << 'EOF'
#!/bin/bash
set -e
...
echo "Creating rocker_scripts/install_python.sh..."
cat > rocker_scripts/install_python.sh << 'EOF'
#!/bin/bash
set -e
...
echo "Creating rocker_scripts/install_s6init.sh..."
cat > rocker_scripts/install_s6init.sh << 'EOF'
#!/bin/bash
set -e
...
echo "Creating rocker_scripts/install_shiny_server.sh..."
cat > rocker_scripts/install_shiny_server.sh << 'EOF'
#!/bin/bash
set -e
...
# Create S6 init scripts
echo "Creating cont-init.d/01_set_env..."
cat > cont-init.d/01_set_env << 'EOF'
#!/usr/bin/with-contenv bash
...
# Create S6 service scripts
echo "Creating services.d/shiny-server/run..."
cat > services.d/shiny-server/run << 'EOF'
#!/usr/bin/with-contenv bash
## Run the entrypoint script that downloads from S3 and launches the app
exec /srv/shiny-server/shiny-server-entrypoint.sh
EOF

echo "Creating services.d/shiny-server/finish..."
cat > services.d/shiny-server/finish << 'EOF'
#!/bin/bash
exec /bin/true
EOF

# Create ODBC configuration files
echo "Creating app-runtime/artifacts/odbc.ini..."
cat > app-runtime/artifacts/odbc.ini << 'EOF'
[ODBC Data Sources]
...
echo "Creating app-runtime-rshiny/artifacts/odbc.ini..."
cp app-runtime/artifacts/odbc.ini app-runtime-rshiny/artifacts/odbc.ini

echo "Creating app-runtime/artifacts/jdbc.ini..."
cat > app-runtime/artifacts/jdbc.ini << 'EOF'
...
echo "Creating setup-build.sh..."
cat > setup-build.sh << 'EOF'
#!/bin/bash
...
# Create test-docker-build.sh
echo "Creating test-docker-build.sh..."
...
echo "2. Replace cloudguard-root.crt with your actual certificate"
echo "3. Review and modify configuration files if needed"
echo "4. Build the image: docker build -t shiny-ubuntu:24.04-s3 ."
echo "5. Test: ./test-docker-build.sh"
echo
echo "Happy building! 🚀"
